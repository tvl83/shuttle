...
export const MobileCosmostationProvider = class MobileCosmostationProvider implements MobileWalletProvider {
  id: string = "mobile-cosmostation";
  name: string = "Cosmostation - WalletConnect";
  ...
  async init(): Promise<void> {
    ...
    this.walletConnect.on("connect", async (error, payload) => {
      ...
      if (peerMetaName !== "Cosmostation") {
        this.walletConnect?.killSession();
        throw new Error(
          `Invalid provider, peerMetaName: ${peerMetaName} doesn't match the expected peerMetaName: Cosmostation`,
        );
      }
  ...
  async connect({
    ...
    return {
      ...
      iosUrl: `cosmostation://wc?${this.walletConnect.uri}`,
      androidUrl: `intent://wc?${this.walletConnect.uri}#Intent;package=wannabit.io.cosmostaion;scheme=cosmostation;end;`,
    };
  }
